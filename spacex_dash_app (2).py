...
launch_site_options = [{'label': value, 'value': value} for value in launch_sites]
...
def plot_payload_scatter(entered_site,sliderval):
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= sliderval[0]) & (spacex_df['Payload Mass (kg)'] <= sliderval[1])]
    #filtered_df = spacex_df
    if entered_site == 'ALL':
